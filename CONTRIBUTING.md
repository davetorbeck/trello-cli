...
This repository uses [changesets](https://github.com/changesets/changesets) to manage package versioning. When adding a new feature, run `bunx changeset add` from the root directory. This will prompt you for a short description of the change and let you interactively select which packages you've edited, and if there are major, minor or patch level changes.
...
1. Bump versions by aggregating unprocessed changesets using `bunx changeset version`
2. Commit all changes
3. Publish packages using `bunx changeset publish`
